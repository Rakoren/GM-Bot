...
if (!manifestPath) {
  console.error("Usage: node tools/check-sourceref-ch4.mjs <path-to-manifest.json>");
  process.exit(2);
}
...
const mustHaveTypes = new Set(["background", "species", "table"]);
let ok = true;
...
console.log("sourceRef check OK (Chapter 4)");
