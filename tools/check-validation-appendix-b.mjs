...
if (!validationPath) {
  console.error("Usage: node tools/check-validation-appendix-b.mjs <path-to-validation.json>");
  process.exit(2);
}
...
const report = {
  validation: validation.title ?? "appendix-b.validation",
...
for (const shard of manifest.shards ?? []) {
  ...
  if (!doc) continue;
  if (shard.type === "creature") {
    requireFields(
      doc,
      shard,
      ["id", "name", "chapter", "status", "abilities", "traits", "actions", "sourceRef"],
      "creature.core_fields"
    );
  }
  if (shard.type === "table") {
    requireFields(
