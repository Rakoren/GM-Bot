...
if (!validationPath) {
  console.error("Usage: node tools/check-validation-ch6.mjs <path-to-validation.json>");
  process.exit(2);
}
...
const report = {
  validation: validation.title ?? "chapter-06.validation",
...
for (const shard of manifest.shards ?? []) {
  ...
  if (shard.type === "item") {
    requireFields(
      doc,
      shard,
      ["id", "name", "chapter", "status", "item_type"],
      "item.core_fields"
    );
    if (doc.status === "complete" && !hasSourceRef(doc)) {
      report.errors.push({
        id: "item.sourceRef.required",
        message: `Complete item missing sourceRef in ${shard.id}`,
      });
    }
  }
