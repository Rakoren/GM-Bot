...
const path = 'admin/public/wizard.js';
const text = fs.readFileSync(path, 'utf8');
const start = text.indexOf('function parseGoldCost');
const end = text.indexOf('function showMessage');

if (start === -1 || end === -1) {
  throw new Error('markers missing');
}

const newBlock = `function parseGoldCost(cost) {
  if (!cost) return 0;
