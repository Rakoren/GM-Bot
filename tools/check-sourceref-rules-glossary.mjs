...
if (!manifestPath) {
  console.error(
    "Usage: node tools/check-sourceref-rules-glossary.mjs <path-to-manifest.json>"
  );
  process.exit(2);
}
...
const mustHaveTypes = new Set(["glossary_term", "table"]);
let ok = true;
...
console.log("sourceRef check OK (Rules Glossary)");
