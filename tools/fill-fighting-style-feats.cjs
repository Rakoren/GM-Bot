...
const startIdx = lines.findIndex((l) => l.trim() === 'Fighting Style Feats');
const endIdx = lines.findIndex((l) => l.trim() === 'Epic Boon Feats');
if (startIdx === -1 || endIdx === -1) {
  console.error('Fighting Style Feats section not found.');
  process.exit(1);
}
...
const fightingFeats = featFiles
  .map((file) => {
    const p = path.join(featsDir, file);
    const data = JSON.parse(fs.readFileSync(p, 'utf8').replace(/^\uFEFF/, ''));
    return { file, data };
  })
  .filter(({ data }) => data.category === 'Fighting Style');

const featNames = fightingFeats.map(({ data }) => data.name);
...
const parsePrereq = (block) => {
  for (const { line } of block) {
    const m = line.match(/^Fighting Style Feat\s*\(([^)]+)\)/);
  ...
  const benefitLines = block
    ...
    .filter(
      (l) =>
        l &&
        !l.startsWith('--') &&
        l.trim() !== 'Fighting Style Feats' &&
        l.trim() !== 'These feats are in the Fighting Style category.'
    );

  while (benefitLines.length && benefitLines[0].trim() === '')
    benefitLines.shift();
  if (benefitLines.length) benefitLines.shift(); // feat name
  if (benefitLines[0] && benefitLines[0].startsWith('Fighting Style Feat'))
    benefitLines.shift();
...
for (let i = 0; i < sorted.length; i += 1) {
  ...
  const nextIdx = i + 1 < sorted.length ? sorted[i + 1].idx : sectionWithPages.length;
  ...
  const featEntry = fightingFeats.find((f) => f.data.name === name);
...
console.log('Fighting Style feats updated:', sorted.length);
