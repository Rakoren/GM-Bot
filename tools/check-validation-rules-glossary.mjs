...
if (!validationPath) {
  console.error("Usage: node tools/check-validation-rules-glossary.mjs <path-to-validation.json>");
  process.exit(2);
}
...
const report = {
  validation: validation.title ?? "rules-glossary.validation",
...
for (const shard of manifest.shards ?? []) {
  ...
  if (!doc) continue;
  if (shard.type === "glossary_term") {
    requireFields(
      doc,
      shard,
      ["id", "name", "chapter", "status", "description", "sourceRef"],
      "glossary_term.core_fields"
    );
  }
  if (shard.type === "table") {
    requireFields(
