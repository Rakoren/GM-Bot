...
if (!validationPath) {
  console.error("Usage: node tools/check-validation-ch5.mjs <path-to-validation.json>");
  process.exit(2);
}
...
const report = {
  validation: validation.title ?? "chapter-05.validation",
...
for (const shard of manifest.shards ?? []) {
  ...
  if (shard.type === "feat") {
    requireFields(
      doc,
      shard,
      ["id", "name", "chapter", "status", "category", "repeatable"],
      "feat.core_fields"
    );
    if (doc.status === "complete" && !hasSourceRef(doc)) {
      report.errors.push({
        id: "feat.sourceRef.required",
        message: `Complete feat missing sourceRef in ${shard.id}`,
      });
    }
  }
