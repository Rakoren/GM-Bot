...
if (!validationPath) {
  console.error("Usage: node tools/check-validation-ch4.mjs <path-to-validation.json>");
  process.exit(2);
}
...
const report = {
  validation: validation.title ?? "chapter-04.validation",
...
for (const shard of manifest.shards ?? []) {
  ...
  if (shard.type === "background") {
    requireFields(
      ...
      "background.core_fields"
    );
    if (!hasSourceRef(doc)) {
      report.errors.push({
        id: "background.sourceRef.required",
        message: `Missing or invalid sourceRef in ${shard.id}`,
      });
    }
  }

  if (shard.type === "species") {
    requireFields(
      doc,
      shard,
      [
    ...
    if (!hasSourceRef(doc)) {
      report.errors.push({
        id: "species.sourceRef.required",
        message: `Missing or invalid sourceRef in ${shard.id}`,
      });
    }
  }

  if (shard.type === "origin_feat") {
    requireFields(doc, shard, ["id", "name", "chapter", "status"], "origin_feat.core_fields");
    if (doc.status === "complete" && !hasSourceRef(doc)) {
      report.warnings.push({
        id: "origin_feat.sourceRef.recommended",
        message: `Complete origin feat missing sourceRef in ${shard.id}`,
      });
    }
  }
